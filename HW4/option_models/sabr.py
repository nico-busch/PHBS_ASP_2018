...
class ModelHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, spot, texp=None, cp_sign=1, setval=False, is_vol=True):
        ...
        if(is_vol):
            ...
            vol = [self.bsm_model.impvol(price_or_vol3[i], strike[i], spot, texp, cp_sign) for i in range(len(price_or_vol3))]
        
        def fun(x):
            s = np.exp(x[0])
            a = np.exp(x[1])
            r = 1/(1+np.exp(-x[2]))
            return vol - bsm_vol(strike, spot, texp, sigma=s, alpha=a, rho=r)
        
        x = sopt.root(fun, [0, 0, 0])
        s = np.exp(x.x[0])
        a = np.exp(x.x[1])
        r = 1/(1+np.exp(-x.x[2]))
...
class ModelNormalHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, spot, texp=None, cp_sign=1, setval=False, is_vol=True):
        ...
        if(is_vol):
            ...
            vol = [self.normal_model.impvol(price_or_vol3[i], strike[i], spot, texp, cp_sign) for i in range(len(price_or_vol3))]
        
        def fun(x):
            s = np.sqrt(x[0]**2)
            a = np.sqrt(x[1]**2)
            r = (2*x[2])/(1+x[2]**2)
            return vol - norm_vol(strike, spot, texp, sigma=s, alpha=a, rho=r)
...
class ModelBsmMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        Z_temp = np.random.normal(size=(n_steps, self.n_samples, strike.size, 2))
        Z1 = Z_temp[:, :, :, 0]
        ...
        forward_t = np.exp(np.log(spot) + np.cumsum(sigma_t * np.sqrt(self.time_step) * Z1 - 0.5 * sigma_t**2 * self.time_step, axis=0))
